...
import { ValidationResultObject } from '../../../types'
...
export default function NewUser() {
  ...
  const [validationResults, setValidationResults] = useState<ValidationResultObject>({
    userId: null,
    familyNames: null,
    givenNames: null,
    emailAddress: null,
  ...
  const canSubmit = useMemo(
    () => userId && givenNames && familyNames && emailAddress && roles.length !== 0 && invalidCount === 0,
    [userId, givenNames, familyNames, emailAddress, roles, invalidCount]
  )
  const handleSubmit = async () => {
    try {
      const existingRecord = (await axios.get(`${process.env.REACT_APP_AUTH_SERVICE}/users/${userId}`)).data
      if (existingRecord) {
        setSaveError(`User ID ${userId} already exists! Please choose another ID.`)
        return
      }
      const response = await axios.post(`${process.env.REACT_APP_AUTH_SERVICE}/users`, {
        userId,
  ...
  return (
    <>
      ...
      <div className="col-xs-12 col-md-6 col-lg-4">
        <form className={`form ${invalidCount > 0 ? 'was-validated:invalid' : ''}`}>
          <StandardInput
            label="User ID"
            name="userId"
            value={userId}
            ...
            onValidationChange={(name, isValid) => setValidationResults({ ...validationResults, [name]: isValid })}
            isRequired={true}
          />
          <StandardInput
            name="givenNames"
          ...
          <div className="mb-3">
            ...
            <select
              ...
              value={roles}
              onChange={(event) => setRoles(Array.from(event.target.selectedOptions).map((o) => o.value))}
              size={3}
              multiple={true}
            >
          ...
          <div>
            ...
            <button type="button" className="btn btn-outline-secondary mx-2" onClick={() => history.goBack()}>
              Cancel
            </button>
          </div>
          {saveError !== null ? <div className="alert alert-danger my-3">{saveError}</div> : <></>}
        </form>
      </div>
    </>
  )
