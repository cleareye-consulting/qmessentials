...
export default function EditUser() {
  ...
  useEffect((): void | (() => void) => {
    ...
    ;(async () => {
      try {
        const user = (await axios.get(`${process.env.REACT_APP_AUTH_SERVICE}/users/${userId}`)).data
        if (!cancel) {
        ...
        console.error(error)
        setErrorMessage('Unable to retrieve user from database. Please try again, or contact support.')
      }
    })()
    return () => (cancel = true)
  }, [userId])
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error(error)
      setErrorMessage('Error saving user. Please try again, or contact support.')
    }
  }
